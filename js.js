...
//Запрет на ввод всего кроме русских букв, пробелов, дефисов
$('#ve').on("keyup", function() {
	this.value = this.value.replace(/[^А-Яа-яЁё\s+$+-]/g, '');
});

//создать объект для городов компьютера и человека
var cityMachine = [];
var cityHuman = [];
var checkLastLetter = 0;

//запросить у игрока город
$('#but').on("click", checkLastLeterFunc);

//проверить начинается ли он с той буквы на которую закончил компьютер
function checkLastLeterFunc(){
	var a = $("#ve").val();
	if(checkLastLetter == 0) {
		init(a); 
		$("#human").empty();
		$("#human").append("<h3>Человек</h3>");
		$("#machin").empty();
		$("#machin").append("<h3>Компьютер</h3>");
	}
	else if(a.toLowerCase().charAt(0) == checkLastLetter) init(a);
	else {
		alert("Слово компьютера заканчивалось на другую букву!");
	}
}
...
//запрос ("город "+a) - плохо
//запрос без пробела (a+"город") - средне
//запрос ("city "+a) - средне
//запрос (a+"city") - хорошо

//поиск через ajax запрос
/*
var necessaryCity, coordinat;
var cor = [];
$.ajax({
   url:"https://geocode-maps.yandex.ru/1.x/?format=json&geocode="+a+"city"+"",
    dataType: 'json'        
}).done(function(data) {
    var mass = data.response.GeoObjectCollection.featureMember;
    if (mass.length == 0) alert("Такого города нет!");
    for(var i = 0; i< mass.length; i++){
    	if(mass[i].GeoObject.metaDataProperty.GeocoderMetaData.kind == "locality" || mass[i].GeoObject.metaDataProperty.GeocoderMetaData.kind == "province") 
    	{
    		necessaryCity = mass[i];
    		coordinat = necessaryCity.GeoObject.Point.pos;
    		var num;
    		for(var i = 0; i<coordinat.length; i++){
    			if(coordinat.charAt(i) == " ") num = i;
    		}
    		cor =[(+coordinat.slice(num+1)), (+coordinat.slice(0,num))];
    		break; 
    	}
    }
});
...
//проверка на locality - отлично

    // Поиск координат центра заданного города.
    ymaps.geocode(a).then(function (res) {
    	var firstGeoObject = checkLocality(res);
        // если города не существует сообщаем об этом
        if(firstGeoObject === 0){
            alert("Такого города нет!");
            return;
        } else {
        	//иначе внести в базу
        	a = firstGeoObject.properties._K.name;
        	if(check(a)){
        		cityHuman.push(a);
        		$("#ve").val("");
        		$("#human").append("<li>"+a+"</li>");
        		addPoint(firstGeoObject.geometry.getCoordinates(), a);
        		var machineWord = getWordForMachin(a);
				if(machineWord){
					cityMachine.push(machineWord);
					$("#machin").append("<li>"+machineWord+"</li>");
					$("#info").empty();
					$("#info").append("<h4>Слово компьютера:</h4>");
					$("#info").append("<h5>"+machineWord+"</h5>");
					$("#info").append("<h3>Счёт "+cityHuman.length+" : "+cityMachine.length+"</h3>");

				}
        	} 
        	else{
        		alert("Такой город уже был в игре!");
        		$("#ve").val("");
        	}	
        }
    }, function (err) {
        // Если геокодирование не удалось, сообщаем об ошибке.
        alert("Ошибка получения информации, попробуйте ещё!");
        alert(err.message);
    });
}

function checkLocality(res){
	var firstGeoObject = 0;
    //на карте города у которых названия такие же как у стран не помечены "locality"
    //проверяем такие города
    for(var j = 0; j<cityHowCountry.length; j++){
        if(res.geoObjects.get(0).properties._K.name == cityHowCountry[j])
        {
            firstGeoObject = res.geoObjects.get(0);
            break;
        }
    } 
    //проверяем точно ли мы нашли город
    for(var i = 0; i<res.metaData.geocoder.found; i++){
        if(res.geoObjects.get(i).properties._K.metaDataProperty.GeocoderMetaData.kind == "locality" || res.geoObjects.get(i).properties._K.metaDataProperty.GeocoderMetaData.kind == "province")
    	 	{
				firstGeoObject = res.geoObjects.get(i);
				break;
    	 	}
    }
...
//проверить не повторяется ли он
function check(a){
	for(var i = 0; i<cityMachine.length; i++){
		if(a.toLowerCase() == cityMachine[i].toLowerCase()){
			return 0;
		} 
	}
	for(i = 0; i<cityHuman.length; i++){
		if(a.toLowerCase() == cityHuman[i].toLowerCase()){
			return 0;
		} 
	}
	return 1;
...
//проверить последнюю букву и найти соответствие в списке
// исключения ъ ь ы й
function getWordForMachin(a){
	var lastLet = chekOnWrongLetter(a);
	for(var i = 0; i<city.length; i++){
		if(city[i].toLowerCase().charAt(0) == lastLet){
			if(check(city[i])) {
				checkLastLetter = chekOnWrongLetter(city[i]);
				var myGeocoder = ymaps.geocode(city[i]);
				myGeocoder.then(
    				function (res) {
    					var firstGeoObject = checkLocality(res);
    					if(firstGeoObject === 0){
    						alert("К сожалению не могу показать свой город на карте.");
    					} else {
        				addPoint(firstGeoObject.geometry.getCoordinates(), city[i]);
        				}
    				},
    				function (err) {
       					alert('К сожалению не могу показать свой город на карте.');
...
function chekOnWrongLetter(a){
	var nameCity = a.toLowerCase();
	var lastLet = (nameCity.charAt(nameCity.length-1));
	if(lastLet == "ъ" || lastLet == "ь" || lastLet == "ы" || lastLet == "й"){
		lastLet = (nameCity.charAt(a.length-2));
	}
	return lastLet;
}
...
function addPoint(xy, name){
   	myMap.geoObjects.add(new ymaps.Placemark(xy, {
...
$('#speech').on("click", startRecognizer);

//ввод по средствам голоса
function startRecognizer(){
    if ('webkitSpeechRecognition' in window) {
      ...
      recognition.start();
    } else alert('webkitSpeechRecognition не поддерживается данным браузером');
}


$('#speech').on("click", startRecognizer);

//функция для выведения результатов
/*
function tableResult(){
	...
	$("#machin").empty();
	$("#machin").append("<h3>Компьютер</h3>");
	cityHuman.forEach(function(el){
		$("#human").append("<li>"+el+"</li>");
	});
	cityMachine.forEach(function(el){
		$("#machin").append("<li>"+el+"</li>");
	});
	$("#info").append("<h3>Счёт "+cityHuman.length+" : "+cityMachine.length+"</h3>");
}
*/

//города у которых названия совпадают со страной
//нет значения "locality"
var cityHowCountry=[
'Алжир',
'Ватикан',
'Гватемала',
...
//Решил подключить базу данных городов, так-как на мой взгляд это
//делает игру более интересной, чем при самостоятельном вводе названий.
//Предпочел бы подключить базу данных, как базу данных используя Node.js
//Но набор идёт на курс по JS, поэтому не стал рисковать.
var city = [
(6, 4, 5, 'Балларат'),
(7, 4, 5, 'Бендиго'),
(8, 4, 5, 'Варрнамбул'),
